...
import re
mydb=connection.MySQLConnection(user='root',host='localhost',password='admin',database='snm0nr12')
app=Flask(__name__)
...
def viewfile(fid):
    ...
    try:
        ...
        storedfile_data=cursor.fetchone() #(1,'Mysql','filedata','2026-06-12'),
        cursor.close()
    except Exception as e:
        print('MYsql Error:',str(e))
        flash('Could not fetch the files details')
...
def search():
    if not session.get('user'):
        ...
        return redirect(url_for('login'))
    try:
        searchdata=request.args.get('query') #m
        strg=['A-Za-z0-9']
        pattern=re.compile(f'^{strg}',re.IGNORECASE)
        if pattern.match(searchdata):
            try:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('select notesid,title,created_at from notesdata where notesid like %s or title like %s or description like %s or  created_at like %s',[searchdata+'%',searchdata+'%',searchdata+'%',searchdata+'%'])
                allnotesdata=cursor.fetchall() #[(1,'Mysql','2026-06-12'),(2,'Python','2026-06-12'),]
                print(allnotesdata)
                cursor.close()
            except Exception as e:
